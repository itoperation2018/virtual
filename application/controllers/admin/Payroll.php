...
class Payroll extends Admin_controller
{
  public function __construct()
    ...
    public function table(){
     ...
     public function manage_salary_table(){
        if (!has_permission('items', '', 'view')) {
            ajax_access_denied();
        }
        $this->perfex_base->get_table_data('manage_salary');
    }
     public function employee_salary_table(){
        if (!has_permission('items', '', 'view')) {
            ajax_access_denied();
        }
        $this->perfex_base->get_table_data('employee_salary');
    }
     public function generate_payslip_table(){
        if (!has_permission('items', '', 'view')) {
            ajax_access_denied();
        }
        $this->perfex_base->get_table_data('generate_payslip');
    }
     public function payroll_summary_table(){
        if (!has_permission('items', '', 'view')) {
            ajax_access_denied();
        }
        $this->perfex_base->get_table_data('payroll_summary');
    }


   public function advance_salary_table(){
        if (!has_permission('items', '', 'view')) {
            ajax_access_denied();
        }
        $this->perfex_base->get_table_data('advance_salary');
    }

     public function provident_fund_table(){
        if (!has_permission('items', '', 'view')) {
            ajax_access_denied();
        }
        $this->perfex_base->get_table_data('provident_fund');
    }
   public function overtime_table(){
        if (!has_permission('items', '', 'view')) {
            ajax_access_denied();
        }
        $this->perfex_base->get_table_data('overtime');
    }


   public function advance_salary_manage()
    {
        if (has_permission('items', '', 'view')) {
            if ($this->input->post()) {
                ...
                if ($data['itemid'] == '') {
                    ...
                    $id      = $this->payroll_model->add_advance_salary($data);
                    $success = false;
                    $message = '';
                    if ($id) {
                        $success = true;
