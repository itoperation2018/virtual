...
class Payroll_model extends CRM_Model
{
    ...
    {
        $columns = $this->db->list_fields('salary_template');
        $rateCurrencyColumns = '';
        ...
        if (is_numeric($id)) {
            $this->db->where('salary_template.id', $id);
    ...
    {
        unset($data['itemid']);
        $columns = $this->db->list_fields('advance_salary');
        $this->load->dbforge();
        ...
        foreach($data as $column => $itemData){
            if(!in_array($column,$columns) ){
                ...
                $this->dbforge->add_column('advance_salary', $field);
            }
        }
        ...
        if ($insert_id) {
            logActivity('New Invoice Item Added [ID:' . $insert_id . ', ' . $data['id'] . ']');
    ...
    public function edit($data)
    {
        ...
        $this->db->update('salary_template', $data);
