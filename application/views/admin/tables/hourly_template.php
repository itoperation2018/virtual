...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
       ...
       if($aColumns[$i] == 'date_in'){
            ...
            $_data = '<a href="#" data-toggle="modal" data-target="#hourly_template_modal" data-id="'.$aRow['id'].'">'.$_data.'</a>';    
        }
    ...
    if(has_permission('items','','edit')){
        $options .= icon_btn('#' . $aRow['id'], 'pencil-square-o', 'btn-default', array(
            'data-toggle' => 'modal',
            'data-target' => '#hourly_template_modal',
            'data-id' => $aRow['id'],
            ));
    }
    if(has_permission('items','','delete')){
       $options .= icon_btn('hourly_template/delete/' . $aRow['id'], 'remove', 'btn-danger _delete');
   }
   $row[] = $options;
